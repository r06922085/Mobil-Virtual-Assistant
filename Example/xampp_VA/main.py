...
class main(object):
    ...
    def test(self, args): 
        ...
        #http
        self.https(model_do, command_dic);
            
    def https(self, model, command_dic):    
        #setting file name
        file_name_ask = 'ask.txt'
        file_name_ans = 'ans.txt'
        
        #runing
        lines = len(open(file_name_ask, 'r').readlines())
        print('running...')
        while True:
            lines_update = len(open(file_name_ask, 'r').readlines())
            if lines_update != lines:
                lines = lines_update;
                
                #openning file
                f = open(file_name_ask, 'r').read().split('\n');
                f = np.asarray(f)
                w = open(file_name_ans, 'a')
                
                #prdicting
                respond_do, respond_do_digit = model.predict(str(f[-2]))
                res_do = ''
                for i in respond_do:
                    res_do += command_dic[int(i)]
                print(res_do)
                
                #writing into answer.txt
                w.write(res_do)
                w.write('\n')
                w.close();
